...
export default function RecipeGrid({ recipes, favorites, toggleFavorite }) {
    ...
    <section>
       ...
       <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-8">
        {/*
          ...
          Mengganti .map((...) => (...)) menjadi .map((...) => { ... return (...) })
        */}
        {recipes.map((recipe, index) => {
          
          // 1. Logika dipindahkan ke dalam blok fungsi {}
          const isFavorited = favorites.some(fav => fav.id === recipe.id);
          
          // 2. JSX dikembalikan (return) secara eksplisit
          return (
            <div 
              key={recipe.id} 
            ...
            >
              ...
              <div className="relative bg-white/15 backdrop-blur-xl border border-white/25 rounded-2xl md:rounded-3xl overflow-hidden shadow-lg md:shadow-2xl shadow-blue-500/5 hover:shadow-blue-500/15 transition-all duration-500 cursor-pointer group-hover:scale-105 group-hover:bg-white/20">
                <button
                  ...
                  className="absolute top-4 right-4 z-20 bg-white/70 backdrop-blur-sm p-2 rounded-full transition-all duration-300 hover:bg-white"
                >
                  {/* 3. Menambahkan Ikon Heart dan menerapkan logika isFavorited */}
                  <Heart className={`w-5 h-5 ${isFavorited ? 'text-red-500 fill-current' : 'text-slate-500'}`} />
                </button>
